...
const Skill = ({ name, x, y, nivel }) => {
  ...
  return (
    <motion.div
      ...
      onMouseEnter={!isTouchDevice ? () => setShowDetail(true) : undefined}
      onMouseLeave={!isTouchDevice ? () => setShowDetail(false) : undefined}
      onClick={isTouchDevice ? handleToggleDetail : undefined}
      style={{ zIndex: 10 }}
    >
      {name}
      ...
      {showDetail && (
        <div
          className="absolute top-full left-0 mt-2 bg-white dark:bg-gray-800 shadow-lg py-2 px-4 rounded-md xs:py-1 xs:px-2 sm:px-3 sm:py-1.5 md:px-4 md:py-2 lg:px-4 lg:py-2 text-gray-800 dark:text-gray-200 font-bold text-xs sm:text-sm md:text-base lg:text-base"
          style={{
            zIndex: 1,
          }}
        >
